...
# ESRGAN 工具路径：优先本项目的 tools，回退到 VideoSniffer
_OWN_TOOLS = Path(__file__).parent / "tools" / "realesrgan"
_SNIFFER_TOOLS = Path(__file__).parent.parent / "VideoSniffer" / "tools" / "realesrgan"


def _find_esrgan_exe() -> str:
    for base in (_OWN_TOOLS, _SNIFFER_TOOLS):
        exe = str(base / "realesrgan-ncnn-vulkan.exe")
        if os.path.exists(exe):
            return exe
    return str(_OWN_TOOLS / "realesrgan-ncnn-vulkan.exe")
...
def _find_models_dir() -> str:
    for base in (_OWN_TOOLS, _SNIFFER_TOOLS):
        ...
        if os.path.isdir(models):
            return models
    return str(_OWN_TOOLS / "models")


_ESRGAN_EXE = _find_esrgan_exe()
